...
wave_line2, = ax_wave.plot([], [], color='#2196f3', linewidth=2, label='Channel 2 (500Hz)')
ax_wave.legend(loc='upper right')


slider_ax2 = plt.axes([0.15, 0.16, 0.7, 0.04])
duty_slider2 = Slider(
    ax=slider_ax2,
    label="Duty 2 (%) ",
    valmin=0, valmax=100,
...
def update_display(val):
    ...
    t1, v1 = generate_pwm_wave(d1, freq=PWM_FREQ)
    wave_line.set_data(t1 * 1000, v1)
    
    t2, v2 = generate_pwm_wave(d2, freq=500) 
    wave_line2.set_data(t2 * 1000, v2)

    avg_v = compute_average_voltage(d1)
...
duty_slider2.on_changed(update_display)
